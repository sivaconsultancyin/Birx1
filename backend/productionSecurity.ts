...
export function validateJsonObject(req: Request, res: Response, next: NextFunction) {
  if (!req.body || typeof req.body !== 'object' || Array.isArray(req.body)) return res.status(400).json({ error: 'Invalid JSON request body' });
  next();
}
...
export function requireHttps(req: Request, res: Response, next: NextFunction) {
  ...
  if (!(req.secure || forwarded === 'https')) return res.status(426).json({ error: 'HTTPS is required in production' });
  next();
}
